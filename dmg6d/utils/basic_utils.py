...
class Basic_Utils():
    ...
    def read_lines(self, p):
        with open(p, 'r') as f:
            lines = [
                ...
                line.strip().replace('/workspace/DATA/', 'datasets/linemod/') for line in f.readlines()
                # line.strip() for line in f.readlines()
            ]
        return lines
    ...
    def lm_draw_points_kp(self, img_id, folder_name, obj_id, obj_name, pred_RT, p3ds):
        ...
        #show_kp_img = cv2.imread(os.path.join('/workspace/REPO/pose_estimation/ffb6d/LineMod_Vis/ape',img_id+'.png'))
        show_kp_img = cv2.imread('datasets/linemod/Linemod_preprocessed/data/'+str(obj_id).zfill(2)+'/rgb/'+img_id+'.png')
        pred_2ds = self.project_p3d(
            pred_p3ds.cpu().numpy(), 1000.0, K='linemod'
        )
        ...
        # imshow("kp: cls_id=%d" % cls_id, show_kp_img)
        if not os.path.exists(os.path.join('vis/LineMod_Vis_kps',obj_name)):
            os.makedirs(os.path.join('vis/LineMod_Vis_kps',obj_name))
        
        cv2.imwrite(os.path.join('vis/LineMod_Vis_kps',obj_name,img_id+'.png'), show_kp_img)
    ...
    def lm_draw_points(self, img_id, folder_name, obj_id, obj_name, pred_RT, p3ds):
        ...
        show_kp_img = self.draw_p2ds(show_kp_img, pred_2ds, r=radius, color=color,alpha=0.7)
        # imshow("kp: cls_id=%d" % cls_id, show_kp_img)
        if not os.path.exists(os.path.join('vis/LineMod_Vis',obj_name)):
            os.makedirs(os.path.join('vis/LineMod_Vis',obj_name))
        
        cv2.imwrite(os.path.join('vis/LineMod_Vis',obj_name,img_id+'.png'), show_kp_img)
    ...
    def lm_draw_points_depth(self, img_id, folder_name, obj_id, obj_name, pred_RT, p3ds):
        ...
        show_kp_img = cv2.imread('datasets/linemod/Linemod_preprocessedd/data/'+str(obj_id).zfill(2)+'/depth/'+img_id+'.png')
        ...
        cv2.imwrite(os.path.join('vis/LineMod_Vis',obj_name,img_id+'_depth.png'), show_kp_img)
    ...
    def save_points(self, img_id, folder_name, obj_name, pred_RT, gt_RT, p3ds):
        ...
        np.savetxt('train_log/'+folder_name+'/'+obj_name+'/eval_results_gt/'+img_id+'_pred.txt', pred_p3ds)
        np.savetxt('train_log/'+folder_name+'/'+obj_name+'/eval_results_gt/'+img_id+'_gt.txt', gt_p3ds)
    ...
    def cal_add_cuda_icp(
        ...
        gt_p3ds = torch.mm(p3ds, gt_RT[:, :3].transpose(1, 0)) + gt_RT[:, 3]
        # apply ICP
        icp_R, icp_t = icp(pred_p3ds, gt_p3ds, tolerance=0.000001)
        # Convert to PyTorch tensors of type float and move to CUDA
        icp_R = torch.from_numpy(icp_R).float().cuda()
        icp_t = torch.from_numpy(icp_t).float().cuda()
        icp_p3ds = torch.mm(pred_p3ds, icp_R[:, :3].transpose(1, 0)) + icp_t
    ...
    def cal_adds_cuda_icp(
        ...
        # apply ICP 
        icp_R, icp_t = icp(pd, gt, tolerance=0.000001)
        # Convert to PyTorch tensors of type float and move to CUDA
        icp_R = torch.from_numpy(icp_R).float().cuda()
        icp_t = torch.from_numpy(icp_t).float().cuda()
        icp_p3ds = torch.mm(pd, icp_R[:, :3].transpose(1, 0)) + icp_t
