...
from models.SwinDePose_Without_NV_Ablation import SwinDePose
...
def save_checkpoint(state, filename="checkpoint", epoch_num=None, loss=None):
    # filename1 = "{}_epoch_{}_loss_{}.pth.tar".format(filename, epoch_num, loss)
    filename2 = "{}.pth.tar".format(filename, epoch_num)
        ...
        for start_epoch in tqdm.tqdm(range(n_epochs)):
            ...
            if test_loader is not None:
                ...
                if val_loss < best_loss:
                    best_loss = val_loss
                    if opt.local_rank == 0:
                        save_checkpoint(checkpoint_state(self.model, self.optimizer, val_loss, start_epoch, it), \
                                        filename=self.checkpoint_name, epoch_num=start_epoch, loss=best_loss)
                        print("----------------------------------------------------------------------------------")
                        print(colored("Saved one checkpoint at {} , epoch: {} ,  loss: {}".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), start_epoch, best_loss), 'green'))
                        print("----------------------------------------------------------------------------------")
                # Early Stopping
