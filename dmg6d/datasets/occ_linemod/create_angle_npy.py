...
################################################### For pbr data ###########################################
for scene_id in tqdm.tqdm(range(1)):
    scene_id = 2
    scene_id = str(scene_id).zfill(6)
    
    for item_name in tqdm.tqdm(range(1214)):
        item_name = str(item_name).zfill(6)
        
        # with Image.open(os.path.join('Occ_LineMod','train_pbr',scene_id,"depth/{}.png".format(item_name))) as di:
        with Image.open(os.path.join('Occ_LineMod','test',scene_id,"depth/{}.png".format(item_name))) as di:
            dpt_mm = np.array(di)
        ...
        if args.vis_img:
            ...
            # # cv2.imwrite(img_file, rgb_s)
            # print('Please look at /workspace/DATA/Occ_Linemod/data/your_class/angles_or_signed_itemname.png')
            img_file = os.path.join('Occ_LineMod','test','vis_nrm_angles_{}.png'.format(item_name))
            cv2.imwrite(img_file, rgb_nrm)
            # img_file = os.path.join('/workspace/DATA/Linemod_preprocessed/data',args.cls_num,'vis_signed_{}.png'.format(item_name))
            # cv2.imwrite(img_file, rgb_s)
            print('Please look at Occ_Linemod/data/your_class/angles_or_signed_itemname.png')
            exit()
            
            
        if not os.path.exists(os.path.join('Occ_LineMod','test',scene_id,'pseudo_nrm_angles')):
            os.makedirs(os.path.join('Occ_LineMod','test',scene_id,'pseudo_nrm_angles'))
        rgb_file = os.path.join('Occ_LineMod','test',scene_id,'pseudo_nrm_angles','{}'.format(item_name))
        np.savez_compressed(rgb_file, angles=rgb_nrm)
        print('Finish {} data generation!!'.format(item_name))
