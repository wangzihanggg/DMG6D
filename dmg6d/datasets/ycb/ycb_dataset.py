...
class Dataset():
    ...
    def __init__(self, dataset_name, DEBUG=False):
        ...
        if dataset_name == 'train':
            self.add_noise = True
            self.path = os.path.join(self.opt.data_root,'dataset_config/','train_data_list_20230706.txt')
            ...
            self.path = os.path.join(self.opt.data_root,'dataset_config/','test_data_list_20230706.txt')
            self.all_lst = bs_utils.read_lines(self.path)
    ...
    def get_item(self, item_name):
        ...
        with np.load(os.path.join(self.root, item_name+'-pseudo_nrm_angles.npz')) as data:
            angles = data['angles']
        ...
        cld_rgb_nrm = np.concatenate((cld, nrm_angles_pt, nrm_pt), axis=1).transpose(1, 0)
        ...
        nrm_angles = np.transpose(nrm_angles, (2, 0, 1))
        ...
        #         imshow("rz_pcld_%d_rnd" % ip, srgb1)for i in range(3):
        #     scale = pow(2, i+1)
        #     nh, nw = h // pow(2, i+1), w // pow(2, i+1)
        #     ys, xs = np.mgrid[:nh, :nw]
        #     xyz_lst.append(xyz_lst[0][:, ys*scale, xs*scale])
