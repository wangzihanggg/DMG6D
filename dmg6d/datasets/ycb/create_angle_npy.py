...
ycb_K2 = np.array([[1077.836, 0., 323.7872],
                   [0., 1078.189, 279.6921],
                   [0., 0., 1.0]], np.float32)
trainlist = np.loadtxt(os.path.join('datasets/ycb/YCB_Video_Dataset/dataset_config/train_data_list_20230706.txt'), dtype='str')
testlist = np.loadtxt(os.path.join('datasets/ycb/YCB_Video_Dataset/dataset_config/test_data_list_20230706.txt'), dtype='str')
...
for item_name in tqdm.tqdm(trainlist):
    ...
    if data_type != 'data_syn':
        ...
        with Image.open(os.path.join('datasets/ycb/YCB_Video_Dataset', data_type, f_idx, i_idx + "-depth.png")) as di:
            dpt_um = np.array(di)
        ...
        rgb_file = os.path.join('datasets/ycb/YCB_Video_Dataset', data_type, f_idx, i_idx + '-pseudo_nrm_angles')
        np.savez_compressed(rgb_file, angles=rgb_nrm)

    else:
        K = ycb_K1
        with Image.open(os.path.join('datasets/ycb/YCB_Video_Dataset', data_type, f_idx + "-depth.png")) as di:
            dpt_um = np.array(di)
        ...
        rgb_file = os.path.join('datasets/ycb/YCB_Video_Dataset', data_type, f_idx + '-pseudo_nrm_angles')
        np.savez_compressed(rgb_file, angles=rgb_nrm)
